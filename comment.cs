...
namespace DAL
{
    public class comment
    {
        ...
        public int GetWorkCount(int id)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select count(1) from Comment where workId=@wordId";
        ...
        public int Add(Model.Comment comment)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into Comment (commentID,workId,userName,CommentContent,CommentTime)" +
                    "values(@commenrID,@workId,@userName,@CommentContent,@CommentTime);";
                sql += "SELECT @@IDENTITY";
        ...
        public int Update(Model.Comment comment)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update Comment set CommentContent=@CommentContent,CommentYome=@CommentContent where commentID=@commentID";
